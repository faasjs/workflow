...
> = BaseContext<TName, TExtend> & {
  ...
  createRecord<TName2 extends keyof Steps>(
    recordProps: {
      stepId: TName2
      action: StepRecordAction
      trx?: K.Transaction
    } & Partial<StepRecord<TName2>>
  ): Promise<any>
  updateRecord<TName3 extends keyof Steps>(
    recordProps: {
      stepId: TName3
      action: StepRecordAction
      id: string
      trx?: K.Transaction
    } & Partial<StepRecord<TName3>>
  ): Promise<any>
}
...
export function buildActions<TName extends keyof Steps>(props: {
  ...
  async function save() {
    ...
    else props.record.summary = props.record.data
  ...
  const previous = {
    ...
    user: props.user,
    ancestorIds: [...(props.record.ancestorIds || []), props.record.id].filter(
      Boolean
    ),
  }
  ...
  async function createRecord<TName extends keyof Steps>(
    ...
    if (!props.record.id && !props.saved) await save()
    return await invoke({
